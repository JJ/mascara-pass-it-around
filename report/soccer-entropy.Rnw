...
This data was used in several technical reports and posters \cite{futbol2005,bundio2007exploraciones,redesvsredes}, applying for the first time, as far as we know, social network analysis to team sports. However, comparing the dataset with others such as the one shown in  \cite{spatial-and-temporal-entropies}, we can observe that the amount of passes is roughly one tenth of what is considered usual in professional matches (which is usually several hundreds). So it might be the case that this dataset is not really valid, or includes only a sample of the network, and not the whole network. This might explain some unusual patterns found in it.
