...
The rest of the paper is organized as follows: next we will examine the state of the art in the area of using social network analysis in soccer; this will be followed in Section \cite{sec:mm} by an examination of the data used in this paper and how it's been obtained. Next we will analyze this data, first separately and then compare it between different competitions, if possible with the same team, focusing on the diverse performance of the Spanish selection in 2004 Euro cup and the 2010 world cup. Finally, we will discuss these results and expose our conclusions.
